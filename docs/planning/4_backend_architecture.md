...
| **2** | **User & entitlement** | `user-service` (today; was `user-api` in earlier drafts) | Profiles, **subscriptions**, **entitlements**, verified Clerk webhook projection, internal user ids; **PostgreSQL** plus Redis profile cache. **Browser clients and Clerk webhooks never call this service over HTTP**—they go through the **BFF**; the BFF reaches user-service over **Nest TCP** (`@nestjs/microservices`). This service is **TCP-only** in local development; no REST listener is bound. Maps to **Identity** in the overview catalog for _application_ identity, while Clerk handles _authentication_. |
...
| `apps/user-service`   | User/entitlement domain; **TCP-only** for BFF RPC and verified Clerk projection                 | Grows with subscriptions and entitlements.             |
...
2. **Move domain + data-access libs** first; keep BFF calling in-process or through **Nest TCP** until wire-up is stable (today: **TCP-only** to `user-service` for domain RPC).
